...
def read_and_check_config(
    ...
    if not (
        ...
        and (isinstance(config["tweet_readme_more"], bool))
    ):
        valid_config = False
    elif not (
...
def what_to_tweet(
    ...
    url = entry["link"]
    read_more = variables.tweet_readme_more
    # title_and_domain = entry["title"] + " - " + domainize(url, remove_www=True)
    title_and_domain = entry["title"]
...
def get_api_keys() -> tuple[str, ...]:
    ...
    try:
        # Get the necessary tokens from the rss-bot.env file. To hardcode the API keys, substitute the os.getenv('') with a string of the API keys.
        if check_if_exists_in_directory("rss-bot.env"):
            load_dotenv(r"rss-bot.env")
        else:
...
# Simpler times: https://github.com/vishalnandagopal/rss-bot/blob/c699e6d1814484ef64d8e5e7c3e562bad33ea90c/app.py
